...
BigInt* createBigInt(long initialLength){
...
void addBigInt(BigInt* bigInt,int num){
  int i=1,carry=0;
  while(num){ 
...
void subtractBigInt(BigInt* a,BigInt* b){
  int prod,j=0;
...
void setBigInt(BigInt* bigInt,int num){
  int i=1;
  while(num){
...
void bigIntTrim(BigInt *num) {
    while (num->size > 1 && num->digits[num->size- 1] == 0) {
        num->size--;
    }
}

void multiplyBigInt(BigInt *a, BigInt *b){
...
int main(){
  BigInt* newInt = createBigInt(30);
  setBigInt(newInt,21389);
  BigInt* newInt2 = createBigInt(30);
  setBigInt(newInt2,1121200);
  subtractBigInt(newInt,newInt2);
  printBigInt(newInt);
